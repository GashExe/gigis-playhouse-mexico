...
import { ArrowLeft, ChartLineUp, ArrowUp, Sparkle } from "@phosphor-icons/react/dist/ssr";
      ...
      <div>
        ...
        {cycle && (
          <p className="mt-2 text-sm text-muted">
            Avance del ciclo <span className="font-semibold text-ink">{cycle.label}</span>. En
            cada actividad ves con qué empezó y con qué va cerrando, del 1 al 4 (4 es
            logrado).
          </p>
        )}
      </div>
                    ...
                    {g.entries.map((e) => (
                      <li key={e.recordId} className="relative">
                        ...
                        <p className="text-xs text-muted">
                          {e.initialScore != null || e.finalScore != null
                            ? `Inicial ${e.initialScore ?? "—"} · Final ${e.finalScore ?? "—"}`
