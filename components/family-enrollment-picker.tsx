...
export function FamilyEnrollmentPicker({
  ...
  const [selected, setSelected] = useState<Set<string>>(
    () => new Set(programs.filter((p) => p.enrolled).map((p) => p.id)),
  );
  ...
  /** Con qué actividad ya palomeada choca esta (null si convive con todas). */
  function clashWith(p: PickerProgram): PickerProgram | null {
    for (const id of selected) {
      if (id === p.id) continue;
      const otro = byId.get(id);
      if (!otro) continue;
      if (findSlotClash(p.slots, otro.slots)) return otro;
    }
    return null;
  }
  ...
  /** Por qué NO se puede palomear (null = sí se puede). */
  function blockedReason(p: PickerProgram): string | null {
    ...
    if (p.occupied >= p.studentCapacity) {
      return "Ya no hay lugares. Puedes pedir lugar en la lista de espera.";
    }
    const choque = clashWith(p);
    if (choque) {
      const label = slotsLabel(p.slots);
  ...
  function toggle(p: PickerProgram) {
    ...
    setSelected((prev) => {
      ...
      if (next.has(p.id)) {
        ...
        if (blockedReason(p)) return prev;
        next.add(p.id);
      }
      return next;
    });
  ...
  function send() {
    startTransition(async () => {
      const result = await submitEnrollmentSelection([...selected]);
      setState(result);
      if (result?.ok) setConfirming(false);
    });
  }

  const elegidas = [...selected]
    .map((id) => byId.get(id))
      ...
      {visibles.length === 0 ? (
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {visibles.map((p) => {
            ...
            const horario = slotsLabel(p.slots);
            ...
            return (
              <li key={p.id}>
                <button
                  ...
                  className={`flex h-full w-full flex-col gap-2 rounded-[var(--radius-card)] border p-4 text-left shadow-[var(--shadow-sm)] transition-colors ${
                    marcada
                      ? "border-primary bg-primary-weak/30"
                ...
                >
                  <div className="flex items-start gap-3">
                    {/* La casilla es decorativa: quien manda es el aria-pressed del botón. */}
                    <span
                      aria-hidden
                ...
                <ul className="space-y-1 rounded-[var(--radius-control)] bg-surface-2 px-3 py-2 text-sm text-ink">
                  {elegidas.map((p) => (
                    <li key={p.id} className="flex items-center gap-2">
                      ...
                      {slotsLabel(p.slots) && (
                        <span className="text-xs text-muted">{slotsLabel(p.slots)}</span>
                      )}
                    </li>
                  ))}
                </ul>
